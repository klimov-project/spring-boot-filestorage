...
export const FileOperationsProvider = ({ children }) => {
  ...
  const nameAlreadyExists = (path) => {
    let fltrd = folderContent.filter(
      (obj) => obj.name === extractSimpleName(path),
    );
  ...
  const identicalTasks = (task1) => {
    ...
    let filtered = pendingTasks.filter(
      (task) => task.operation.source === task1.operation.source,
    );
    ...
    const uploadTasks = allNames.map((source) => {
      ...
      if (nameAlreadyExists(source)) {
        task = {
          ...
          status: 'error',
          message:
            "'" + extractSimpleName(source) + "'" + ' уже существует в папке',
        };
      }
      let files = innerFolders[source]
        ? innerFolders[source]
  ...
  const executeUploadTasks = async (uniqueTasks) => {
    ...
    try {
      await Promise.all(
        uniqueTasks.map(async ({ task, files }) => {
  ...
  const deleteObject = (objects) => {
    let deleteTasks = objects.map((path) =>
      createTask(path, null, 'delete', 'В очереди на удаление'),
    );
  ...
  const moveObjects = (sourceObjects, target) => {
    const moveTasks = sourceObjects.map((source) =>
      createTask(
        source,
        target + extractSimpleName(source),
        'move',
        'В очереди для перемещения',
      ),
    );
  ...
  const moveObjectInternal = (sourcePath, targetPath) => {
    const moveTasks = sourcePath.map((source) =>
      createTask(source, targetPath, 'move', 'В очереди для перемещения'),
    );
    let uniqueTasks = moveTasks.filter((task) => !identicalTasks(task));
  ...
  const copyObjects = (sourceObjects, target) => {
    const copyTasks = sourceObjects.map((source) =>
      createTask(
        source,
        target + extractSimpleName(source),
        'copy',
        'В очереди для копирования',
      ),
    );
  ...
  const copyObjectInternal = (sourceObjects, target) => {
    const copyTasks = sourceObjects.map((source) =>
      createTask(source, target, 'copy', 'В очереди для копирования'),
    );
    let uniqueTasks = copyTasks.filter((task) => !identicalTasks(task));
  ...
  const downloadObjects = (objectPath) => {
    console.log('downloadObjects 111 ', objectPath);
    ...
    console.log('downloadObjects 222 downloadTask ', downloadTask);
    ...
    setTasks((prevTasks) =>
      prevTasks.map((inTask) =>
        inTask.id === task.id
          ? {
              ...inTask,
              message: 'Скачиваем... ' + bytes(speed) + '/с',
            }
          : inTask,
      ),
    );
  ...
  const executeDownloadTask = async (downloadTask) => {
    ...
    try {
      console.log('executeDownloadTask: downloadTask - ', downloadTask);
  ...
  async function executeRename(task) {
    try {
      ...
      const newName = extractSimpleName1.endsWith('/')
        ? extractSimpleName1.slice(0, -1)
        : extractSimpleName1;
