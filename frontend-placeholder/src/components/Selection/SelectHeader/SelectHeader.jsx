...
export const SelectHeader = () => {
  ...
  const handleClose = useCallback((event) => {
    ...
    const contextPopper = elementsUnderCursor.find((elem) =>
      elem.classList.contains('MuiPopper-root'),
    );
    if (contextPopper) return;

    const tileUnderCursor = elementsUnderCursor.find((elem) =>
      elem.classList.contains('selectable'),
    );
    const shouldStartDrag =
      tileUnderCursor && selectedIds.includes(tileUnderCursor.dataset.id);
  ...
  const handleContextMenu = useCallback(
    (event) => {
      ...
      const cont = elementsUnderCursor.find((elem) =>
        elem.classList.contains('MuiContainer-root'),
      );
      ...
      const selectableItem = elementsUnderCursor.find((elem) =>
        elem.classList.contains('selectable'),
      );
    ...
    >
      <Toolbar
        sx={{
          ...
          border: '1px solid',
          background:
            'linear-gradient(90deg, rgba(28,50,163,1) 0%, rgba(16,113,195,1) 100%)', // Градиент для прогресса
      ...
      >
        ...
        >
          {allowToCut && (
            <>
              <MenuItem disabled={selectedIds.length === 0} onClick={handleCut}>
                <ListItemIcon>
