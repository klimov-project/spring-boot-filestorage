...
export const CustomBread = () => {
  const { folderPath, loadFolder } = useStorageNavigation();
  ...
  return (
    ...
    >
      ...
      {folderPath.length > 1 && (
        <NavigateNextIcon
          sx={{ color: 'text.secondary', height: '32px' }}
          fontSize="small"
        />
      )}

      <Breadcrumbs
        sx={{ ml: 1, minWidth: 'max-content' }}
          ...
          return (
            <Chip
              ...
              sx={{
                ...
                transition: 'text-shadow 0.3s ease',
                '&:hover': {
                  boxShadow: !lastElement
                    ? '0 0 10px rgba(25, 118, 210,1)'
                    : '',
                  backgroundColor: lastElement ? 'info.main' : '',
                },
              }}
              onClick={() =>
                !lastElement && loadFolder(buildUrl(item, folderPath))
              }
            />
          );
        })}
